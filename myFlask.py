...
def index():
    a_user = db.session.query(User).filter_by(email='sstrilk1@uncc.edu').one()
...
def update_post(note_id):
    if request.method == 'POST':
        ...
        a_user = db.session.query(User).filter_by(email='sstrilk1@uncc.edu').one()
        my_post = db.session.query(Note).filter_by(id=note_id).one()
        return render_template('edit_selected_question.html', note=my_post, user=a_user)
